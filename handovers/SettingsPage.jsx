...
function ModuleDisableModal({ moduleLabel, onCancel, onDisable, onMigrateAndDisable }) {
  ...
  const handleMigrateAndDisable = async () => {
    ...
    try {
      ...
      // Brief pause so user sees the result before modal closes
...
export default function SettingsPage({ writeFile, readFile, listTree, settings, saveSettings }) {
  ...
  // Commits a module toggle to form + persisted settings
  ...
  const handleModuleToggle = (moduleId, moduleLabel, currentlyEnabled) => {
    const willDisable = currentlyEnabled
    // Enabling always proceeds immediately
    if (!willDisable) {
  ...
  // Runs migration then disables; returns summary for modal result display
  const handleMigrateAndDisable = async () => {
    const result = await migrateEntityTasks({ readFile, writeFile, listTree })
    await commitModuleToggle(pendingDisable.id, false)
    // Give modal time to show result before we close it
