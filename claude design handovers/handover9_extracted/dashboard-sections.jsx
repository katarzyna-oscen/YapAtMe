...
function PersonCard({ person, closeness, handlers, isDragging, isOver }) {
  ...
  const tasksText = `${openTasks} ${openTasks === 1 ? "task" : "tasks"}`;
  return (
    <div
      {...handlers}
      style={{
    ...
    >
      ...
      {/* Line 3 — last mention (left) + open task count (right) */}
      <div style={{
        display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10,
        fontSize: 11, color: "var(--text-very-dim)", fontVariantNumeric: "tabular-nums",
      }}>
        <span style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", minWidth: 0 }}>
          {lastMentionText}
        </span>
        <span style={{ flex: "0 0 auto" }}>{tasksText}</span>
      </div>
    </div>
  );
}
...
//  Ideas
// ────────────────────────────────────────────────────────────
function IdeasSection({ ideas, onNavigate, onStatusChange }) {
  const [localItems, setLocalItems] = React.useState(MEM_IDEAS);
  const items = ideas || localItems;
  const updateStatus = (id, status) => {
    if (onStatusChange) onStatusChange(id, status);
    else setLocalItems((arr) => arr.map((p) => p.id === id ? { ...p, status } : p));
  };
        ...
        {items.map((idea, i) => {
          return (
            <div key={idea.id} style={{
              ...
              onClick={() => onNavigate && onNavigate({ type: "idea", id: idea.id })}
            ...
            >
              ...
              <IdeaStatusPill status={idea.status} onChange={(s) => updateStatus(idea.id, s)} small />
            </div>
          );
        })}
      </div>
