...
function SidebarFileRow({ item, active, onClick, onAction }) {
  ...
  const showMenuBtn = !!onAction && !item.noMenu;
  ...
  return (
    <div
      ...
      {item.icon && (
        <span style={{ display: "inline-flex", color: active ? "var(--active)" : "var(--text-very-dim)", marginLeft: -18, marginRight: 2, flex: "0 0 auto" }}>
          <Icon name={item.icon} size={13} />
        </span>
      )}
      <span style={sidebarStyles.fileLabel}>{item.label}</span>
      {typeof item.badge === "number" && item.badge > 0 && (
        <span style={sidebarStyles.navItemBadge}>{item.badge}</span>
      )}
      {showMenuBtn &&
...
function Sidebar({ view, onNavigate, onItemAction, inbox, notes, archive, tasks, people, ideas, backlogCount, plansCount }) {
  const isDashboard = view.type === "dashboard";
  const isTasks = view.type === "tasks";
  const isPlans = view.type === "plans";
  ...
  return (
    <aside style={sidebarStyles.root} data-screen-label="Sidebar">
      ...
      <nav style={sidebarStyles.nav}>
        ...
        <div
          style={sidebarStyles.navItem(isPlans)}
          onClick={() => onNavigate({ type: "plans" })}
          onMouseEnter={(e) => { if (!isPlans) { e.currentTarget.style.background = "var(--panel-2)"; e.currentTarget.style.color = "var(--text)"; } }}
          onMouseLeave={(e) => { if (!isPlans) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "var(--text-dim)"; } }}>
          ...
          items={[
            {
              ...
              noMenu: true,
              badge: backlogCount,
              active: view.type === "ideas-backlog",
            },
            ...(ideas || MEM_IDEAS).map((i) => ({
              id: i.id,
              label: i.title,
              active: view.type === "idea" && view.id === i.id,
            })),
          ]}
          onItemClick={(it) =>
            it.id === "ideas-backlog"
              ? onNavigate({ type: "ideas-backlog" })
              : onNavigate({ type: "idea", id: it.id })
          }
          onItemAction={(action, it) => onItemAction(action, "idea", it)}
        />
        <SidebarSection
          title="Archive"
